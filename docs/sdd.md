...
Cada feature nueva (`"sdd": true` en `specs/feature-list.yml`) tiene una carpeta dedicada en cuanto deja `PENDING`:
...
El `feature-name` coincide con el campo `name` de `feature-list.yml`.
...
| `BLOCKED`      | Atascado. Razón en `specs/progress/current.md`.                      |

## La puerta de aprobación humana

El flujo automático se detiene **una vez**: cuando el `spec_author` termina
sus tres archivos, marca la feature como `SPEC_READY` y para. El humano lee `specs/features/<feature>/` y dice "aprobado" (o pide cambios).

Solo entonces el `leader` transiciona `SPEC_READY → IN_PROGRESS` y lanza el `implementer`.
...
El `implementer` documenta el mapa en `specs/progress/impl_<name>.md`:
