...
import type { IProductRepository, IOrderRepository } from "../../interfaces";
import { inject } from "../../providers";

function placeOrderAction(productRepository: IProductRepository, orderRepository: IOrderRepository) {
  const logger = Logger("placeOrderAction");
  ...
  return async (
    ...
    if (!result.isOk) {
      logger.log('result::err', result.error.message);
      return {
        success: false,
...
export const placeOrder = placeOrderAction(
  inject("productRepository") as IProductRepository,
  inject("orderRepository") as IOrderRepository,
);
