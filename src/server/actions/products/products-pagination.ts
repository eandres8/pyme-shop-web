...
function getPaginatedProductsWithImagesAction(productRepository: IProductRepository) {
  const logger = Logger('getPaginatedProductsWithImagesAction');

  return async (props: Props): Promise<TPaginateData<Product[]>> => {
...
export const getPaginatedProductsWithImages = getPaginatedProductsWithImagesAction(inject("productRepository") as IProductRepository);
